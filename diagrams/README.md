...
### **7. 07_unified_architecture_event_mapping.png**
**Title:** Unified Architecture — Event Mapping + Step Functions

**What it shows:**
- Users / Apps → API Gateway → Agent Orchestrator hub
- Direct answer path for normal synchronous requests that return immediately to the caller
- Response Router path that brings direct answers, human-reviewed answers, and safe fallback replies back to the user
- RAG ingestion path: S3 clinical documents → chunking/embeddings → OpenSearch Serverless vector store
- RAG query path: agent query → Bedrock Knowledge Base retrieval → top passages injected into the model prompt
- Event Layer branch: EventBridge bus, event-specific routing, SQS queues, and Step Functions workflows
- Memory & Retrieval branch: DynamoDB, S3, Knowledge Base, Titan Embeddings, OpenSearch Serverless
- Observability branch: CloudWatch Logs, Metrics / Alarms, X-Ray
- Security & Governance branch: IAM, KMS, Secrets Manager, CloudTrail, WAF / Shield

**Best for:**
- Explaining how the agent orchestrator fans out into event processing and multi-step workflows
- Explaining how answers flow back from the agent, a human reviewer, or a fallback route
- Showing the difference between immediate HTTP responses and delayed human-in-the-loop responses
- Showing how the vector store is used during retrieval, not as the final response store
- Separating document ingestion from query-time semantic retrieval for interview explanations
- Showing the relationship between audit events, EventBridge, SQS, and Step Functions
- Architecture interviews where a branching hub-and-spoke diagram is easier to narrate than a linear flow

**Key concepts:**
- EventBridge is the routing hub for emitted audit events
- Normal requests can return through the direct synchronous answer path without waiting on async workflows
- Bedrock Knowledge Base performs semantic retrieval over the OpenSearch vector store and injects the returned passages into the prompt
- SQS buffers durable work for HITL, compliance, safety, and audit ingestion
- Human-reviewed decisions re-enter the response path through the Response Router
- Step Functions handles long-running HITL and ingestion workflows
- The diagram uses the same hub-and-spoke style as the repo's other high-level architecture image
...
### **7. 07_unified_architecture_event_mapping.png**
**Title:** Unified Architecture — Request Pipeline → Event Layer → Step Functions

**What it shows:**
A single end-to-end diagram connecting all four layers in horizontal columns:

1. **Col 1 – ECS Fargate Pipeline**: Client → ALB → 9-layer security pipeline → DynamoDB → Response. Each layer has a coloured event badge showing which audit event it emits.
2. **Col 2 – EventBridge**: `healthcare-agent-bus` with one routing row per event — `put_events()` source, filter DetailType, and all fan-out targets (WAF Lambda, SIEM, CloudWatch alarm, SNS, SLA watchdog, etc.). CloudWatch Logs base layer shown at the bottom.
3. **Col 3 – SQS Queues**: 4 queues (`HITL-Review.fifo`, `Compliance`, `Safety-Review`, `Audit-Ingestion`) each showing trigger events, consumer Lambda, and DLQ behaviour.
4. **Col 4 – Step Functions**: SM-1 HITL Review (Standard, 72h `WaitForCallback` + task token, Retry/Catch config, Choice branches) and SM-2 Document Ingestion (Express, Map state parallel chunk embedding).

Horizontal arrows connect pipeline event badges → EventBridge rows → SQS queues → Step Functions entry states.

**Best for:**
- Walk-through interviews showing the full async event-driven architecture in one view
- HITL callback pattern (task token → `SendTaskSuccess`)
- Standard vs Express Step Functions comparison
- "What happens after the agent responds?" questions

**Key concepts:**
- 7 audit events map to EventBridge `DetailType` routing rules
- EventBridge = fan-out; SQS = guaranteed single consumer; SFN = durable workflow
- `WaitForCallback` task token bridges SQS reviewer → Step Functions resume
- Map state in SM-2 fans out parallel chunk processing, waits for ALL

**Generated by:** `diagrams/generate_unified_architecture.py`
...
| 07_unified_architecture_event_mapping | branching hub-and-spoke | High (event + workflow mapping) |
